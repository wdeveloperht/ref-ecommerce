...
use Botble\DataSynchronize\Syncer\SyncColumn;
use Botble\DataSynchronize\Syncer\SyncCounter;
...
class ProductSyncer extends Syncer
{
    ...
    {
        $columns = [
            SyncColumn::make('id')->label('ID'),
            SyncColumn::make('name'),
            SyncColumn::make('description'),
            SyncColumn::make('slug'),
            SyncColumn::make('url')->label('URL'),
            SyncColumn::make('sku')->label('SKU'),
            SyncColumn::make('categories'),
            SyncColumn::make('status'),
            SyncColumn::make('is_featured'),
            SyncColumn::make('brand'),
            SyncColumn::make('product_collections'),
            SyncColumn::make('labels'),
            SyncColumn::make('taxes'),
            SyncColumn::make('image'),
            SyncColumn::make('images'),
            SyncColumn::make('price'),
            SyncColumn::make('product_attributes'),
            SyncColumn::make('import_type'),
            SyncColumn::make('is_variation_default'),
            SyncColumn::make('stock_status'),
            SyncColumn::make('with_storehouse_management'),
            SyncColumn::make('quantity'),
            SyncColumn::make('allow_checkout_when_out_of_stock'),
            SyncColumn::make('sale_price'),
            SyncColumn::make('start_date_sale_price'),
            SyncColumn::make('end_date_sale_price'),
            SyncColumn::make('weight'),
            SyncColumn::make('length'),
            SyncColumn::make('wide'),
            SyncColumn::make('height'),
            SyncColumn::make('cost_per_item'),
            SyncColumn::make('barcode'),
            SyncColumn::make('content'),
            SyncColumn::make('tags'),
            SyncColumn::make('generate_license_code'),
            SyncColumn::make('minimum_order_quantity'),
            SyncColumn::make('maximum_order_quantity'),
        ];

        if ($this->isEnabledDigital) {
            $columns[] = SyncColumn::make('product_type');
        }

        if ($this->isMarketplaceActive) {
            $columns[] = SyncColumn::make('vendor');
        }

        foreach ($this->supportedLocales as $locale) {
            if ($locale['lang_code'] !== $this->defaultLanguage) {
                $columns[] = SyncColumn::make("name_{$locale['lang_code']}")
                    ->label('Name (' . strtoupper($locale['lang_code']) . ')');
                $columns[] = SyncColumn::make("description_{$locale['lang_code']}")
                    ->label('Description (' . strtoupper($locale['lang_code']) . ')');
                $columns[] = SyncColumn::make("content_{$locale['lang_code']}")
    ...
    public function counters(): array
    {
        ...
        return [
            SyncCounter::make()
                ->label(trans('plugins/ecommerce::products.sync.total_items'))
            ...
            SyncCounter::make()
                ->label(trans('plugins/ecommerce::products.sync.total_variations'))
